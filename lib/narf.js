			...
			config.authentication( request, url_object  ).then( function( valid ){
				...
				if( !valid )
					...
					if( func_to_use && config.functions ){
						if( config.functions.POST && config.functions.POST.hasOwnProperty( func_to_use ) && typeof config.functions.POST[ func_to_use ] === 'function' ){
							narf_log( 'executing POST function' );

							to_return = config.functions.POST[ func_to_use ]( { 'body' : obj || null , 'url' : url_object , 'headers' : request.headers }, function( return_object ){
		...
		config.authentication( request, url_object  ).then( function( valid ){
			...
			if( !valid )
				...
				if( func_to_use && config.functions ){
					...
					if( config.functions.GET && config.functions.GET.hasOwnProperty( func_to_use ) && typeof config.functions.GET[ func_to_use ] === 'function' )
						to_return = config.functions.GET[ func_to_use ]( { 'headers' : request.headers, 'url' : url_object }, function( return_object ){
					...
					to_return = ERROR.UNSUPPORTED_FUNCTION;
					console.log( 'Level 2 , ', func_to_use, ' , ' , config.functions );
				}
...
*/
narf_http_server.prototype.addSocket = function( config ){
