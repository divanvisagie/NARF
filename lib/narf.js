...
var page_handler = function( config, request, response ){
	console.log( 'page handler called'.red );
	...
	if ( !config.path )
		throw 'PageServer requires a path';
	else{
...
var api_function_handler = function( api_instance, request, response ){
	...
	api_instance.config.authentication = api_instance.config.authentication || auth_function;
	...
	else if ( url_object.serverfunction && api_instance.config.url_selection || true )
	...
	if ( request.method === 'POST' ){
		...
		request.on( 'end', function(){
			...
			api_instance.config.authentication( request, url_object  ).then( function( valid ){
...
var narf_http_server = function ( config ){
	...
	this.server_handler = function( request, response ){
		...
		if( !self.API_list.hasOwnProperty( func_key ) ){ /* if the key doesn't exist in the service list , work harder to try find it*/

			var delimiter = '/',
			start = 2,
			tokens = request.url.split(delimiter).slice(start),
			result = tokens.join(delimiter);

			console.log( 'Res string'.yellow ,result ,' ', self.API_list.length);
