...
var api_function_handler = function( config, request, response ){
	...
	if ( supportedVerbs.indexOf( request.method.toUpperCase() ) !== -1 ){
		...
		var handle_post = function(){
			...
			if( body_data !== '' && body_data ){ /* We wont always get body data and we dont want
			...
			config.authentication( request, url_object ).then( function( valid ){
				...
				if( !valid ){
					...
					if( func_to_use && config.functions ){
						...
						if( config.functions[request.method.toUpperCase()] &&
							config.functions[request.method.toUpperCase()].hasOwnProperty( func_to_use ) &&
							typeof config.functions[request.method.toUpperCase()][ func_to_use ] === 'function' ){
...
/*
	...
	The HttpServer object wraps the http server and provides a context for all of narfs abstractions
 */
var HttpServer = function ( conf ){
	...
	this.APIList = {}; /* though it is named APIList , this is where all the services for
							this port are stored including page servers*/
			...
			connection.on( 'message', function( message ){ /* the user has sent a message */
				...
				if ( message.type === 'utf8' ){ /* only accept utf8 messages */
					...
					try{
						...
						if ( message_data.serverfunction && config.functions[message_data.serverfunction] &&
							typeof config.functions[message_data.serverfunction] === 'function' ){
				...
				if( config.asc ){
					...
					if( config.close ){
						...
						narf_log( 'WARNING: '.red +
							'narfSocketServer() requires a second callback to handle disconnections if asc is disabled.'.yellow );
					}
				}
			} );
		} catch ( ex ){
