...
exports.startHTTPServer = function ( api_functions, use_sockets ){
	...
	if ( config.port === 'auto' ){ /* handle automatic port detection */
		...
		portastic.find( options, function( err, ports ) { /* retrieve an arrray of ports */
			...
			portastic.test( ports, function( err, data ){
				...
				if( data[0] ){ /* Use first available port */
					...
					start_server( data[0], use_sockets );
				}
				else
		...
		start_server( config.port, use_sockets );
	}
	...
	function start_server( port, sockets ){  //start the appropriate server

		

		if ( !config.https )
			http_server =  http.createServer( server_handler ).listen( port );
		else{
			var fs = require('fs');
